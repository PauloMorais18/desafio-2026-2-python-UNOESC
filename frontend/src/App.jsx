...
function App() {
  const canDeleteDocuments = LOCAL_HOSTNAMES.has(window.location.hostname);
  ...
  async function loadConversationList(token = accessToken) {
    ...
    setChatHistory((current) => body.map((conversation) => {
      ...
      return {
        ...
        messageCount: conversation.quantidadeMensagens ?? existing?.messageCount ?? existing?.messages?.length ?? 0,
      };
    }));
  }
  ...
  async function deleteConversation(conversation, event) {
    ...
    if (typeof conversation.id !== "string") {
      setChatHistory((current) => current.filter((chat) => chat.id !== conversation.id));
      if (currentChatId === conversation.id) startNewChat();
      return;
    }
    if (!window.confirm(`Excluir a conversa "${conversation.title}" e suas mensagens do histórico?`)) return;
    try {
      const response = await fetch(`${API_URL}/conversas/${encodeURIComponent(conversation.id)}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      if (!response.ok && response.status !== 404) {
        const body = await response.json().catch(() => ({}));
        throw new Error(getApiErrorMessage(body.detail, "Não foi possível excluir a conversa."));
      }
      setChatHistory((current) => current.filter((chat) => chat.id !== conversation.id));
      if (currentChatId === conversation.id) startNewChat();
    } catch (error) {
      setLoginError(error instanceof TypeError ? "Não foi possível conectar ao servidor." : error.message || "Não foi possível excluir a conversa.");
    }
  }
  ...
  async function openStatistics(period = statisticsPeriod) {
    ...
    try {
      ...
      const requestedPeriod = typeof period === "string" ? period : statisticsPeriod;
      const normalizedPeriod = ["hoje", "7dias", "30dias", "tudo"].includes(requestedPeriod.trim().toLowerCase())
        ? requestedPeriod.trim().toLowerCase()
        : "hoje";
      const periodQuery = `?periodo=${encodeURIComponent(normalizedPeriod)}`;
  ...
  async function sendMessage(event) {
    ...
    try {
      ...
      const errorMessage = error instanceof TypeError ? "Não foi possível conectar ao servidor. Tente novamente em instantes." : error.message || "Não foi possível enviar sua pergunta.";
      const assistantMessage = { id: Date.now(), author: "assistant", content: errorMessage, time: formatTime() };
      setMessages((current) => [...current, assistantMessage]);
      setChatHistory((current) => current.map((chat) => chat.id === chatId ? { ...chat, messages: [...chat.messages, assistantMessage] } : chat));
    } finally {
  ...
  return (
    <main className="application-shell">
      <aside className="sidebar">
        <div>
          <div className="brand"><div className="brand-symbol" aria-hidden="true"><span /></div><div><strong>UnoAssist</strong><small>Assistente Acadêmico</small></div></div>
          ...
          <nav aria-label="Histórico de conversas">
            ...
            {chatHistory.length ? chatHistory.map((conversation) => <div className="conversation-row" key={conversation.id}><button className={`conversation-item ${activeConversation === conversation.id ? "active" : ""}`} type="button" onClick={() => selectConversation(conversation)}><span className="chat-icon" aria-hidden="true" /><span className="conversation-title">{conversation.title}</span></button><button className="conversation-delete" type="button" aria-label={`Excluir conversa ${conversation.title}`} title="Excluir conversa" onClick={(event) => deleteConversation(conversation, event)}>×</button></div>) : <p className="empty-history">Nenhuma conversa iniciada.</p>}
          </nav>
        </div>
      ...
      <section className="content">
        ...
        {showLogin && <div className="modal-backdrop auth-modal-backdrop" role="presentation" onMouseDown={closeAuth}><section className="modal-card auth-modal" role="dialog" aria-modal="true" aria-labelledby="auth-modal-title" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" type="button" aria-label="Fechar" onClick={closeAuth}>×</button><div className="auth-modal-copy"><h2 id="auth-modal-title">{authMode === "login" ? "Acesse sua conta" : "Crie sua conta"}</h2><p>{authMode === "login" ? "Entre com seu código de aluno e senha para enviar perguntas." : "Use seu código de aluno e defina uma senha com pelo menos 8 caracteres."}</p><button className="auth-switch" type="button" onClick={() => { setAuthMode(authMode === "login" ? "register" : "login"); setLoginError(""); }}>{authMode === "login" ? "Ainda não possui conta? Criar agora" : "Já possui conta? Entrar"}</button></div><form onSubmit={authMode === "login" ? login : register}><input autoFocus value={loginCode} onChange={(event) => setLoginCode(event.target.value)} placeholder="Código do aluno" aria-label="Código do aluno" /><div className="password-field"><input value={loginPassword} onChange={(event) => setLoginPassword(event.target.value)} type={showPassword ? "text" : "password"} minLength="8" placeholder="Senha" aria-label="Senha" /><button type="button" className="toggle-password" onClick={() => setShowPassword(!showPassword)}>{showPassword ? "Ocultar" : "Ver"}</button></div>{authMode === "register" && <input value={passwordConfirmation} onChange={(event) => setPasswordConfirmation(event.target.value)} type={showPassword ? "text" : "password"} minLength="8" placeholder="Confirme sua senha" aria-label="Confirme sua senha" />}<button className="auth-submit" type="submit" disabled={isLoggingIn}>{isLoggingIn ? "Aguarde..." : authMode === "login" ? "Entrar" : "Criar conta"}</button></form>{loginError && <p className="login-error" role="alert">{loginError}</p>}</section></div>}
...
function DocumentationScreen({ documentation, loading, error, onBack, onRefresh }) {
  ...
  return <section className="utility-screen documentation-screen" aria-label="Documentação da API"><div className="utility-header"><div><span className="utility-eyebrow">API UnoAssist</span><h2>Documentação da API</h2><p>Endpoints disponíveis nesta instância do Assistente Acadêmico.</p></div><div><button type="button" onClick={onRefresh}>Atualizar</button><a className="utility-link-button" href={`${API_URL}/docs`} target="_blank" rel="noreferrer">Abrir Swagger</a><button type="button" onClick={onBack}>Voltar ao chat</button></div></div>{loading ? <div className="utility-empty">Carregando endpoints...</div> : error ? <div className="utility-empty utility-error">{error}</div> : <><div className="api-summary"><span>{endpoints.length} endpoints disponíveis</span><code>{API_URL}</code></div><div className="api-endpoints-list">{endpoints.map(({ path, method, operation }) => <article className="api-endpoint" key={`${method}-${path}`}><div className="endpoint-main"><span className={`http-method method-${method.toLowerCase()}`}>{method}</span><code>{path}</code></div><div className="endpoint-details"><strong>{operation.summary || operation.operationId || "Operação da API"}</strong><p>{operation.description || "Endpoint disponível na API UnoAssist."}</p><small>{operation.tags?.join(" · ") || "API"}{operation.requestBody ? " · recebe dados" : ""}</small></div></article>)}</div><div className="documentation-footer">Esta lista é obtida diretamente do contrato OpenAPI. Para exemplos de requisição e resposta, use o botão <strong>Abrir Swagger</strong>.</div></>}</section>;
}
